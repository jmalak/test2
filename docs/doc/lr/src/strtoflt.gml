...
.arg ptr
to
.if '&function' eq 'strtod' .id double
.el .if '&function' eq 'strtof' .id float
.el .if '&function' eq 'strtold' .id long double
representation.
.np
First, &func decomposes the input string into three parts: an initial,
