...
Internet years it would probably span millennia. The origins can be
...
desired. Directory names which include spaces are extremely discouraged
on every host OS.
.np
...
.note build\bin
contains miscellaneous binaries used in the build process, compiled to run
...
.np
.ix 'src'
Following is a brief description of all subdirectories of src. Each subdirectory
...
.note awk
a version of the classic awk text processor, used for building.
.note bdiff
...
.note cfloat
utility library for conversion between various floating-point binary formats.
.note cg
...
.note cl
The C/C++ compiler drivers, wcl and owcc.
.note clib
...
.note d16api
Minimal include files needed for building DOS/16M applications.
.note dbg
the debugger (formerly known as WVIDEO).
.note dig
...
.note disasm
the "new" disassembler supporting a variety of file formats and
instruction sets. Very handy.
.note distrib
this is where installers get built.
.note dlgprs
the dlgprs utility needed to build the source browser.
.note dmpobj
a simple OMF dump utility.
.note dump
a utility to dump various object and executable formats.
.note dwarf
...
.note hcdos
The help compiler for DOS (.ihp) help files.
.note hdr
source files of header files distributed with the compilers.
.note helpcomp
the Windows 3.1 help compiler.
.note hlpview
...
.note link
the Open Watcom linker (wlink), also contains the overlay manager library.
.note mad
Machine Architecture Description used by debugger.
.note make
the make utility (wmake).
.note mathlib
the math library.
.note misc
stuff that didn't fit anywhere else. Not much really.
.note mkdisk
utilities for building the installers.
.note mstools
Microsoft clone tools, front ends for compilers and utilities.
.note omftools
build utilities used to modify OMF object files.
.note ncurses
a version of the ncurses library used by Unix console tools.
.note orl
...
.note profiler
the Open Watcom profiler UI.
.note rc
the Watcom resource compiler (wrc).
.note rcsdll
interface to various revision control systems, used by IDE and editor.
.note re2c
regular expression to C converter, used in C++ compiler build and elsewhere.
.note redist
...
.note sdk
SDK tools like resource editor, dialog editor, or image editor.
.note setupgui
source for the Open Watcom installer user interface.
.note ssl
internal tool used for debugger builds.
.note strip
wstrip utility for detaching or attaching debug information and/or
resources.
.note techinfo
ancient system information utility.
.note touch
a touch utility (wtouch).
.note trap
...
.note wclass
a C++ class library used by the IDE and other projects.
.note wdisasm
...
.note wres
the wres library for loading and manipulating resources.
.note wstub
...
C/C++ and some free disk space to burn (several gigabytes will be needed).
.np
