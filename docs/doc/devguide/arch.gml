...
compile on various people's machines, and allow for easier porting to other
architectures, every project which is developed as part of Open Watcom
...
.us build\mif
) installed. That is taken care of by uncompressing an Open Watcom source
archive or checking out the source repository. Note that MIF stands for
Make Include File.
.*
.section Makeinit
.*
...
.begnote
.note mif_path:
        must point to the directory or directories in which the mif project
	has been installed
.note lang_root:
...
The
.us makeinit
file must be stored in a directory listed in the
.id PATH
environment variable in order to be found by wmake. The .mif files
included from
.us makeinit
and other makefiles are found using a separate
.id mif_path
make variable if their (usually relative) path is not explicitly given.
...
.id cg_dir.
.np
In the typical case, all of your projects use the standard directory structure
under a common directory, you can set
.id dev_dir
and
...
to take any extra action to use it. In fact the default
.us makeinit
can almost always be used without modifications and all critical information
is passed to it through several environment variables.
.np
...
.note host_cpu:
        processor architecture which the resulting executable code will run on.
.note proj_name:
        the project name
.endnote
.np
Valid values for
.id host_cpu
are 386, i86, axp, ppc, mps. These should be self-explanatory, except perhaps
for mps which is a shorthand for MIPS.
Valid values for
.id host_os
...
executables which can run on multiple OSes if appropriate loader stub is
provided. There are also several osi libraries that do not contain any OS
specific code or run-time conventions.
.np
...
macros - these are currently (any additions should be added here):
.begnote
.note $(CC):
The C compiler
.note $(CPP):
...
A project should be able to build either a -d2 level debug (if
...
of crashes or other highly unusual behaviour, a release build should be enough
...
and clashes.
.np
Tools required to build are another issue. In general, the only tools required
to build should be either a previous Open Watcom release or (on a set of supported
platforms) the vendor's standard compiler suite. Anything else needs to be built
from source.
Any small special purpose build tools need to be checked-in in source form
along with the project and built as part of the build process (so that
we don't have to check in zillions of binaries for all supported platforms). An
important future consideration is the ability to build on a different
architecture. Please try and avoid weirdo tools that have no hope of running
on an Alpha or PPC or ARM running NT or Linux.
...
followed by the
.id host_cpu,
if convenient. For example, NT versions for the Alpha AXP should be genned into
an ntaxp directory. If a directory structure which is different than this
...
.np
We are trying to get away from the three-component names and instead
have a separate directory sub-tree for all compilers that generate 386
code, another for all compilers that generate axp code, and so on.
.np
Cross-compilation is also why the macro names are somewhat
counterintuitive &mdash most people would think of the
.id host_os
and
.id host_cpu,
...
.us inc_dirs
macro. This mechanism is now considered obsolete and should not be used.
...
.millust end
Note that for portability, the paths must be included in double quotes and
use forward slashes (not backslashes) as path separators. The compilers
hosted on DOS-like platforms can deal with forward slashes, but Unix based
tools cannot use backslashes as path separators.
...
We use either the Open Watcom C/C++ compilers or the platform vendor's compilers
together with Watcom
.us wmake
...
a ~1GHz machine, although it should finish in half an hour or less on
a ~4GHz system with flash storage. 
.np
...
although that should change in future.
.np
...
for infrastructure.
.note aui
Not a test per se, nevertheless the sample programs in
.us src\aui\sample
...
In
.us cg\test\far16
there is a test exercising the curious and OS/2 specific
.id __far16
...
.note idedemo
Again not a test per se, the 'IDE demo' is a good way to verify basic IDE
functionality.
.note wasm
