...
/* Known subsection names; max length 14 chars */
static struct {
    uint16_t    sst_num;
    const char  *sst_name;
} cv4_sst_names[] = {
...
static  char    *cv4_dir_entry_msg_short[] = {
    "2= ",
    "2  iMod        - module index (1 based)        = ",
    "4  lfo         - offset from base              = ",
    "4  cb          - subsection size               = ",
...
static  char    *cv4_sym_hdr_msg[] = {
    "2    symhash    - index of symbol hash func    = ",
    "2    addrhash   - index of address hash func   = ",
    "4    cbSymbol   - bytes in symbol table        = ",
    "4    cbSymHash  - bytes in symbol hash table   = ",
    "4    cbAddrHash - bytes in address hash table  = ",
    NULL
};


static const char *find_cv4_sst_name( uint16_t sst )
{
    int         i;
    const char  *np = NULL;

    for( i = 0; i < sizeof( cv4_sst_names ) / sizeof( cv4_sst_names[0] ); ++i ) {
        if ( cv4_sst_names[i].sst_num == sst ) {
            np = cv4_sst_names[i].sst_name;
            break;
        }
    }
...
/*
 * dump_cv4_symbols - dump CV4 sstGlobalSym/sstGlobalPub at 'offset'
 * from 'base 'containing 'size' bytes
 */
static void dump_cv4_symbols( unsigned_32 base, uint16_t sst_type,
                              unsigned_32 offset, unsigned_32 size )
/******************************************************************/
{
    cv_sst_global_pub_header    pub_hdr;
//    unsigned_32         read = 0;
//    unsigned_8          name_len;
//    char                name[256];

    if( sst_type == sstGlobalSym )
        Wdputs( "==== sstGlobalSym at offset " );
    else
        Wdputs( "==== sstGlobalPub at offset " );
    Puthex( offset, 8 );
    Wdputslc( "\n" );

    Wlseek( base + offset );
    Wread( &pub_hdr, sizeof( pub_hdr ) );
    Dump_header( &pub_hdr, cv4_sym_hdr_msg );
#if 0
    while( read < size ) {
        Wread( &pub16, sizeof( pub16 ) );
        name_len = (unsigned_8)pub16.name[0];
...
/**************************************************************************/
{
    ...
    case sstGlobalPub:
        if( Debug_options & GLOBAL_INFO ) {
            dump_cv4_symbols( base, sst_index, dir->lfo, dir->cb );
        }
        break;
    }
}
...
/**************************************/
{
    ...
    for( i = 0; i < sst_dir_hdr.cDir; ++i ) {
        ...
        sst_name = find_cv4_sst_name( sst_dir_entry.subsection );
        Wdputs( "  subsection  - type (" );
        if( sst_name ) {
            Wdputs( sst_name );
            pad = 20 - strlen( sst_name );
        } else {
            Wdputs( "unsupported" );
            pad = 9;
        }
        ...
        Dump_header( &sst_dir_entry, cv4_dir_entry_msg_short );
        Wdputslc( "\n" );
