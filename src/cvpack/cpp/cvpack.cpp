...
uint CVpack::FindCSeg( char*& buffer, unsigned_32& length )
/*********************************************************/
{
    uint        cSeg = 0;
...
void CVpack::DoSegMap( char *buffer, unsigned_32 length )
/*******************************************************/
{
...
/************************/
{
    ...
    if ( _aRetriever.ReadSubsection(buffer,length,sstLibraries) ) {
        ...
        _newDir.Insert( sstLibraries, MODULE_INDEPENDENT, 0, 0 );
    }
}
...
/**************************/
{
    ...
    unsigned_32 segmap_length;
    char        *segmap_tmp;
    uint cSeg = FindCSeg( segmap_tmp, segmap_length );

    char        *segmap_buffer = new char[segmap_length];
    memcpy( segmap_buffer, segmap_tmp, segmap_length );
    ...
    DoSegMap( segmap_buffer, segmap_length );
    //cerr << "finished DoSegMap.\n"; cerr.flush();
    delete segmap_buffer;
