...
char *GetErrorMsg( vi_rc err )
{
    ...
    if( err < 0 || err > errCnt ) {
        if( !errCnt )
            MySprintf( strBuff, "Error %d (no message, failed to read errmsg.dat)", err );
	else
            MySprintf( strBuff, "Error %d (no message, error no. out of range)", err );
        return( strBuff );
    }
    msg = GetTokenString( errorList, (int)err );
    if( msg == NULL ) {
        MySprintf( strBuff, "Error %d (error message not found)", err );
        return( strBuff );
    }
    return( msg );
