...
// class a form
{
    ...
    case DW_FORM_indirect:
        ...
        break;
    default:
        formcl = DWR_FORMCL_data;   // Wrong but silences warning
        //TODO: Convince compiler that DWREXCEPT does not return?
        DWREXCEPT( DREXCEP_BAD_DBG_INFO );
    }
    return( formcl );
}
...
// The final call shoud be target else a goose chase
{
    ...
    abbrev = DWRLookupAbbrev( mod, abbrev );
    tag = DWRVMReadULEB128( &abbrev );
    tag = tag;  // Silence warning, 'tag; can be used for debugging
    haschild = DWRVMReadByte( abbrev );
