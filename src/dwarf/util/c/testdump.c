...
static char *getTAG( uint_32 value ) {
    ...
    if( result == NULL ) {
        sprintf( buf, "TAG_%08lx", (unsigned long)value );
        return( buf );
    }
    return( result );
}
...
static char *getFORM( uint_32 value ) {
    ...
    if( result == NULL ) {
        sprintf( buf, "FORM_%08lx", (unsigned long)value );
        return( buf );
    }
    return( result );
}
...
static char *getAT( uint_32 value ) {
    ...
    if( result == NULL ) {
        sprintf( buf, "AT_%08lx", (unsigned long)value );
        return( buf );
    }
    return( result );
}
...
static void dumpInfo( const uint_8 *input, uint length ) {
    ...
    while( p - input < length ) {
        ...
        printf( "Length: %08lx\nVersion: %04x\nAbbrev: %08lx\nAddress Size %02x\n",
            (unsigned long)unit_length, getU16( (uint_16 *)(p+4) ), (unsigned long)abbrev_offset, address_size );
        p += 11;
        while( p - unit_base < unit_length ) {
            printf( "offset %08lx: ", (long)(p - input) );
    ...
    decode_form:
                ...
                case DW_FORM_data4:
                case DW_FORM_ref4:
                    printf( "\t%08lx\n", (unsigned long)getU32( (uint_32 *)p ) );
                    p += sizeof( uint_32 );
                    break;
                case DW_FORM_flag:
                    printf( "\t%s\n", *p++ ? "True" : "False" );
...
static void dumpState( state_info *state ) {
    ...
    printf( "-- file %d addr %08lx line %d column %d",
        state->file, (unsigned long)state->address, state->line, state->column );
    ...
    uint                        length )
{
    ...
    while( p - input < length ) {
        ...
        printf( "prologue_length: 0x%08lx (%lu)\n", (unsigned long)getU32( (uint_32 *)p ), (unsigned long)getU32( (uint_32 *)p ) );
        p += sizeof( uint_32 );
        ...
        while( *p != 0 ) {
            ...
            printf( "file %u: '%s' dir_index %08lx mod_time %08lx length %08lx\n",
                file_index, name, (unsigned long)dir_index, (unsigned long)mod_time, (unsigned long)file_length );
            if( p - input >= length ) return;
        }
        printf( "-- end files --\n");
        p++;
            ...
            if( op_code == 0 ) {
                ...
                case DW_LNE_define_file:
                    ...
                    printf( "DEFINE_FILE %u: '%s' directory %ld mod_time %08lx length %08lx\n",
                        file_index, name, (long)directory, (unsigned long)mod_time, (unsigned long)file_length );
                    break;
                default:
                    printf( "** unknown extended opcode: %02x - %u bytes\n", op_code, op_len );
                    printf( "** losing %lu bytes\n", (unsigned long)(unit_length - ( p - unit_base )));
...
static void dumpARanges( const uint_8 *input, uint length )
{
    ...
    while( p - input < length ) {
        ...
        q = (const uint_8 *)(((uintptr_t)p + tuple_size - 1) & ~(tuple_size - 1));
        padding = q - p;
