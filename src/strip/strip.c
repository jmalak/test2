...
int main( int argc, char *argv[] )
{
    ...
    rc = chsize( fout.h, lseek( fout.h, 0L, SEEK_CUR ) );
#else
    rc = ftruncate( fout.h, lseek( fout.h, 0L, SEEK_CUR ) );
#endif
    if( rc ) {
        Fatal( MSG_WRITE_ERROR, fout.name );
    }
...
static void AddInfo()
{
    ...
    if( res ) {
        ...
        if( read( finfo.h, (void*)&header, sizeof( header ) ) != sizeof( header ) ) {
            FatalDelTmp( MSG_READ_ERROR, finfo.name );
        }
