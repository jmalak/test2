...
#else
/* POSIX.1-2017 no longer defines ecvt() and some systems no longer have it. */
char *my_ecvt( double value, int ndigit, int *decpt, int *sign)
{
    static char buf[16];    /* No need to be reentrant. */
    char        tmp[32];
    int         i;
    int         j;

    /* Restrict the number of digits so it fits into buffer. */
    if( ndigit - 1 > 15 )
        ndigit = 15;
    /* sprintf() does all the hard word. */
    sprintf( tmp, "%.*e", ndigit - 1, value );
    /* If negative, i will be 1 to skip the sign. */
    i = tmp[0] == '-';
    *sign = i;

    /* Copy over, stopping before 'e' and skipping decimal point. */
    for( j = 0; tmp[i] != 'e'; j += (tmp[i++] != '.') )
        buf[j] = tmp[i];
    buf[j] = '\0';

    /* Convert the number after 'e' into radix position. */
    *decpt = atoi( tmp + i + 1 ) + 1;
...
#else
    mant = my_ecvt( value->value, mant_digs, &exp, &sign );
#endif
    if( !isdigit( *mant ) ) {
