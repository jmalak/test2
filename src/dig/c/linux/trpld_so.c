...
char *LoadTrap( char *trapbuff, char *buff, trap_version *trap_ver )
{
    ...
    char                trap_file[_MAX_PATH];

    if( trapbuff == NULL )
        trapbuff = "std";
    for( ptr = trapbuff; *ptr != '\0' && *ptr != ';' && *ptr != ':'; ++ptr )
        ;
    parm = (*ptr != '\0') ? ptr + 1 : ptr;
    PathOpenTrap( trapbuff, ptr - trapbuff, "so", trap_name, sizeof( trap_name ) );
    ...
    if( TrapFile == NULL ) {
        /* Rebuild the trap file name and look for it along the PATH. */
        memcpy( trap_file, trapbuff, ptr - trapbuff );
        trap_file[ptr - trapbuff] = '\0';
        strcat( trap_file, ".so" );
        _searchenv( trap_file, "PATH", trap_name );
        /* If _searchenv() didn't find anything, don't dlopen(). */
        if( *trap_name )
            TrapFile = dlopen( trap_name, RTLD_NOW );
        if( TrapFile == NULL ) {
            puts( dlerror() );
