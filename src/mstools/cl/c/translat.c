...
/***********************************************************************/
{
    ...
    switch( cmdOpts->calling_convention ) {
      case OPT_calling_convention_Gc:   // pascal
        AppendCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-ecp" );
        break;
      case OPT_calling_convention_Gd:   // cdecl
        AppendCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-ecc" );
        break;
      case OPT_calling_convention_Gr:   // fastcall
        AppendCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-ecf" );
        break;
      case OPT_calling_convention_Gz:   // stdcall
        AppendCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-ecd" );
        break;
    }
    ...
    if( cmdOpts->ND ) {
        AppendFmtCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-nd=%s",
                          cmdOpts->ND_value->data );
    }

#ifdef __TARGET_386__
    if( cmdOpts->NM ) {
        AppendFmtCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-nm=%s",
                          cmdOpts->NM_value->data );
    }
#endif

    if( cmdOpts->NT ) {
        AppendFmtCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-nt=%s",
                          cmdOpts->NT_value->data );
    }

    switch( cmdOpts->debug_info ) {
      case OPT_debug_info_Z7:
        AppendCmdLine( compCmdLine, CL_C_OPTS_SECTION, "-hc" );
        status->debugLevel = 2;
        break;
...
/**********************************************************************/
{
    ...
    /*
     * Do this *before* optimization options. If the -d2 option is produced
     * (default with MS -Zi/-Z7 options) then optimizations are completely
     * disabled. However, sometimes it is highly desirable to enable certain
     * optimizations anyway.
     * Note that in MS compilers, the -Zi/-Z7 switch is independent of
     * optimization options and does not disable optimizations like -d2 does.
     */
