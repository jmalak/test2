...
static bool itemlt( void *_a, void *_b )
{
    ...
    /*  The original algorithm compared pointers; unfortunately, in some
     * environments the memory allocations are randomized and yacc produces
     * different results from run to run.
     *  To stabilize the results, we use serial numbers instead. This happens
     * to produce the same results as the original code did in typical
     * environments where subsequent allocations are placed at monotonically
...
#else
    if( a->p.sym ) {
        return( b->p.sym && a[0].p.sym->sn > b[0].p.sym->sn );
    } else {
        return( b->p.sym || a[1].p.pro->pidx > b[1].p.pro->pidx );
    }
#endif
}
