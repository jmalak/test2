    ...
    if( actual_rc == SR_CLOSEST || actual_rc == SR_EXACT ) {
        ...
        if( failed ) {
            printf( "FAILED: %s: cue2:{file=%#x line=%lu addr=%04x:%08lx}\n"
                    ...
                    operation,
                    (unsigned)CueFileId( cue2 ), line2, addr2.mach.segment, (long)addr2.mach.offset,
                    strlen( operation ), "",
                    (unsigned)CueFileId( cue ), line, addr.mach.segment, (long)addr.mach.offset );
        }
    }
}
...
static walk_result Mod2Callback( mod_handle mh, void *_idx )
{
    ...
    if( 1 && ModHasInfo( mh, HK_CUE ) == DS_OK ) {
        printf( "%03d Line Numbers\n"
                ...
                *idx );
        i = 0;
        WalkFileList( mh, File2Callback, &i );
        printf( "\n"
                "\n" );
    ...
    if( 1 && ModHasInfo( mh, HK_TYPE ) == DS_OK ) {
        printf( " %03d Types\n"
                ...
                *idx);
        i = 0;
        WalkTypeList( mh, Type2Callback, &i );
        printf( "\n"
                "\n" );
        need_trailing_newline = 1;
    }
    ...
    if( 1 && ModHasInfo( mh, HK_SYM ) == DS_OK ) {
        printf( "%03d Symbols\n"
                ...
                *idx );
        i = 0;
        WalkSymList( SS_MODULE, &mh, Sym2Callback, &i );
        printf( "\n"
                "\n" );
        need_trailing_newline = 1;
    }
...
static int DumpIt( const char *file, mod_handle mh, process_info *proc )
{
    ...
    if( 1 ) {
        printf( " Global Symbols\n"
                ...
                "------------------------------------------\n");
        i = 0;
        WalkSymList( SS_MODULE, &mh, SymCallback, &i );
        printf( "\n"
                "\n" );
    }
