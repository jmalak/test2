...
static dip_status GetSectInfo( dig_fhandle f, unsigned long *sizes,
...
// Fill in the starting offset & length of the dwarf sections
{
    ...
    *byteswap = FALSE;
    ...
    memset( bases, 0, DR_DEBUG_NUM_SECTS * sizeof( bases[0] ) );
    memset( sizes, 0, DR_DEBUG_NUM_SECTS * sizeof( sizes[0] ) );
...
// Add any global symbol to the hash
{
    ...
    if( !DRIsStatic( sym ) ) {
        ...
        len = len;  // Silence warning, 'len' can be used for debugging
