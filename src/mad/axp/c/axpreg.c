...
*  ========================================================================
*
* Description:  Alpha AXP registers structures and access routines.
*
****************************************************************************/
...
/* The Microsoft C compiler is broken and offsetof does not always produce
 * a constant expression. See x86reg.c for details.
 * For Micrsosoft C, there has to be a special BIT_OFF_R macro for getting
 * at register array members. For everyone else, BIT_OFF_R is almost the same
 * as BIT_OFF.
 */
#ifndef _MSC_VER
#define BIT_OFF_R( which )  (offsetof( mad_registers, axp.r[which] ) * BITS_PER_BYTE)
#else
#define BIT_OFF_R( which )  ((offsetof( mad_registers, axp.r ) + ((which) * sizeof(((mad_registers *)0)->axp.r[0]))) * BITS_PER_BYTE)
#endif
...
#define sublist_r( name, type, reg_set, base, start, len )        \
        { { NAME_##name,                \
            AXPT_##type,                \
            BIT_OFF_R( base )+start,    \
            len,                        \
            1 },                        \
            PAL_all, reg_set##_REG_SET, RS_NONE },
...
static const axp_reg_info       FPCRSubList[] = {
    sublist_r( invd,BYTE, FPU, AR_fpcr, 49, 1 )
    sublist_r( dzed,BYTE, FPU, AR_fpcr, 50, 1 )
    sublist_r( ovfd,BYTE, FPU, AR_fpcr, 51, 1 )
    sublist_r( inv, BYTE, FPU, AR_fpcr, 52, 1 )
    sublist_r( dze, BYTE, FPU, AR_fpcr, 53, 1 )
    sublist_r( ovf, BYTE, FPU, AR_fpcr, 54, 1 )
    sublist_r( unf, BYTE, FPU, AR_fpcr, 55, 1 )
    sublist_r( ine, BYTE, FPU, AR_fpcr, 56, 1 )
    sublist_r( iov, BYTE, FPU, AR_fpcr, 57, 1 )
    sublist_r( dyn, BYTE, FPU, AR_fpcr, 58, 2 )
    sublist_r( sum, BYTE, FPU, AR_fpcr, 63, 1 )
    { NULL }
};
...
{
    switch( ri->bit_start ) {
    case BIT_OFF_R( AR_r31 ):
    case BIT_OFF_R( AR_f31 ):
        *possible_p = NULL;
        *num_possible_p = 0;
        return( MS_FAIL );
    }
...
void            DIGENTRY MIRegUpdateEnd( mad_registers *mr, unsigned flags, unsigned bit_start, unsigned bit_size )
{
    ...
    case BIT_OFF( pal.nt.fir ):
        ...
        break;
    case BIT_OFF_R( AR_sp ):
        MCNotify( MNT_MODIFY_SP, NULL );
        break;
    case BIT_OFF_R( AR_fp ):
