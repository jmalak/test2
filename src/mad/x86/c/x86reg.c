...
/*
 * This horrible crap is only here because the Microsoft C compiler is broken, and
 * always has been. The problem has been there since Microsoft C 6.0 (1990) until
 * at least MSVC 19.40, 30+ years later.
 * The C standard (since C89) clearly says that the offsetof macro expands to
 * an integer constant expression, which is naturaly perfectly good for sizing arrays,
 * initializing, case statement, etc. etc. But for whatever reason Microsoft's offsetof
 * has no trouble with taking the offset of an array within a struct, but refuses to
 * deal with individual array members.
 * So we do it the hard way and calculate the offsets ourselves. Ugh!
 */
#ifndef _MSC_VER
#define FPU_STK( num )  \
    FPU( st##num, EXTENDED, reg[num], 0, 80, NULL, L1 )
#else
#define FPU_REG( name, type, regno, startbit, size, sublist, fpulevel )   \
    REG( FPU, name, type, (offsetof( mad_registers, x86.fpu.reg ) + \
        ((regno) * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] )))*8+startbit,        \
        size, FP, sublist, LX, fpulevel )

#define FPU_STK( num )  \
    FPU_REG( st##num, EXTENDED, num, 0, 80, NULL, L1 )
#endif
...
#else
#define MMX( num )      \
    REG( MMX, mm##num, U64, (offsetof( mad_registers, x86.mmx.mm) + \
        ((num) * sizeof( ((mad_registers *)0)->x86.mmx.mm[0] )))*8,\
        64, MM, MMXSubList##num, L5, LX )
#endif
...
#else
#define XMM( num )      \
    REG( XMM, xmm##num, U128, (offsetof( mad_registers, x86.xmm.xmm ) + \
        ((num) * sizeof( ((mad_registers *)0)->x86.xmm.xmm[0] )))*8,\
    128, XMM, XMMSubList##num, L6, LX )
#endif
...
#else
/* The Microsoft C compiler appears to be deficient and won't
 * accept the forward array declarations without specifying
 * a size. So we hack it by hardcoding the sizes, it's not like
 * they will change very often or at all.
 */
static const x86_reg_info       * const EFLRegList[13];
static const x86_reg_info       * const SWRegList[16];
static const x86_reg_info       * const CWRegList[11];
static const x86_reg_info       * const MXCSRRegList[16];
#endif
...
#define MMX_SUBREG( s, i, reg ) \
    REG( MMX##reg, s##i, s,     \
        (offsetof( mad_registers, x86.mmx.mm) + \
        ((reg) * sizeof( ((mad_registers *)0)->x86.mmx.mm[0] )))*8 + MMXS_##s * i,\
        MMXS_##s, MM, NULL, L5, LX )

#define XMM_SUBREG( s, i, reg ) \
    REG( XMM##reg, s##i, s,     \
        (offsetof( mad_registers, x86.xmm.xmm ) + \
        ((reg) * sizeof( ((mad_registers *)0)->x86.xmm.xmm[0] )))*8 + XMMS_##s * i,\
        XMMS_##s, XMM, NULL, L6, LX )
...
#else
    /* See above for why this ugliness is needed for Microsoft C. */
    case (offsetof( mad_registers, x86.fpu.reg ) + (0 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
    case (offsetof( mad_registers, x86.fpu.reg ) + (1 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
    case (offsetof( mad_registers, x86.fpu.reg ) + (2 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
    case (offsetof( mad_registers, x86.fpu.reg ) + (3 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
    case (offsetof( mad_registers, x86.fpu.reg ) + (4 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
    case (offsetof( mad_registers, x86.fpu.reg ) + (5 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
    case (offsetof( mad_registers, x86.fpu.reg ) + (6 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
    case (offsetof( mad_registers, x86.fpu.reg ) + (7 * sizeof( ((mad_registers *)0)->x86.fpu.reg[0] ))) * 8:
#endif
        if( flags & RF_FPREG ) {
            unsigned    index;
