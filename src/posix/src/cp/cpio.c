...
//    if( _dos_open( src, O_RDONLY, &handle ) ) {
    if( (handle = open( src, O_RDONLY | O_BINARY )) < 0 ) {
    ...
    srcattr = srcattr;
    close( handle );    //_dos_close( handle );
    /*
    ...
    close( cb->inhandle );  //_dos_close( cb->inhandle );
#endif
    if( okay ) {
        FileCnt++;
    }
...
//                bytes != buffsize ) {
    if( read( CBTail->inhandle, buff, buffsize ) != buffsize ) {
...
//          if( _dos_creat( curr->outname, _A_NORMAL, &curr->outhandle ) ) {
            curr->outhandle = open( curr->outname, O_RDWR | O_CREAT | O_TRUNC | O_BINARY, _S_IREAD | _S_IWRITE );
            if( curr->outhandle < 0 ) {
        ...
        while( mb != NULL ) {
            ...
            bytes = write( curr->outhandle, buff, mb->buffsize );
            if( bytes != mb->buffsize ) {
