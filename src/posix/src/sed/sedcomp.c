...
int main( int argc, char *argv[] )
{
    ...
    * -e i\ hello -e "s/$/ world" */
    ...
    /*  In the Watcom C runtime, supplying 'size' of 0 to setvbuf() together
     * with NULL for the buffer pointer leaves the current buffer size unchanged.
     * This behavior is not standardized (or even documented by Watcom).
     *  The allowable range of 'size' is not specified in either Standard C or
     * POSIX. Microsoft's C runtime requires the size to be at least 2.
     * Supplying BUFSIZE should achieve the intent (change the buffering mode
     * but not buffer size) in a portable manner.
     */
    (void)setvbuf( stdout, NULL, _IOLBF, BUFSIZ );  /* Improve reactivity in a pipe */
    if( eargc <= 0 ) {           /* if there are no files specified */
        (void)setvbuf( stdin, NULL, _IOLBF, BUFSIZ );   /* Improve reactivity in a pipe */
