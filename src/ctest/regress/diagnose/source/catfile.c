    ...
    , void* data )              // - not used
{
    ...
    data = data;
#ifdef __UNIX__
    // Use pg.buffer as temporary storage
    strcpy( pg.buffer, tp->text );
    strcpy( path, dirname( pg.buffer ) );
    strcpy( pg.buffer, tp->text );
    strcpy( pattern, basename( pg.buffer ) );
    // Now put the path back in pg.buffer/pg.dir
    strcpy( pg.buffer, path );
    pg.dir = pg.buffer;
#else
