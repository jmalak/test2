...
# Build wtouch; depends on wstrip
[ INCLUDE <SRCDIR>/touch/builder.ctl ]
# Continue with the linker and exe2bin
...
[ INCLUDE <SRCDIR>/dlgprs/builder.ctl ]
[ INCLUDE <SRCDIR>/vi/ctl/builder.ctl ]
[ INCLUDE <SRCDIR>/vi/bind/builder.ctl ]
# The ide2make utility
[ INCLUDE <SRCDIR>/wclass/builder.ctl ]
[ INCLUDE <SRCDIR>/ide/mviper/builder.ctl ]
[ INCLUDE <SRCDIR>/ide/bviper/builder.ctl ]
# Utilities required to build documentation
[ INCLUDE <SRCDIR>/bmp2eps/builder.ctl ]
[ INCLUDE <SRCDIR>/whpcvt/builder.ctl ]
[ INCLUDE <SRCDIR>/hcdos/builder.ctl ]
[ INCLUDE <SRCDIR>/helpcomp/builder.ctl ]
[ INCLUDE <SRCDIR>/ipfc/builder.ctl ]
[ INCLUDE <SRCDIR>/wgml/builder.ctl ]
# Libraries needed for installer tools
[ INCLUDE <SRCDIR>/zlib/builder.ctl ]
[ INCLUDE <SRCDIR>/ziplib/builder.ctl ]
# Tools needed to build installers
[ INCLUDE <SRCDIR>/mkdisk/builder.ctl ]

[ IFDEF <BLD_HOST> UNIX ]
#========================
# The following tools are not required during build but useful
...
[ BLOCK <1> boot ]
#===========================
# Manually copy wmake to the host native bin directory. Must be done
# here since we're not including wmake's builder.ctl for boot builds.
  <CCCMD> <SRCDIR>/make/bootstrp/wmake <RELROOT>/bin/wmake
...
# And the DOS extender stub plus CauseWay.
[ INCLUDE <SRCDIR>/wstub/builder.ctl ]
[ INCLUDE <SRCDIR>/causeway/builder.ctl ]
[ INCLUDE <SRCDIR>/osiloadr/builder.ctl ]
...
[ INCLUDE <SRCDIR>/profiler/builder.ctl ]
[ INCLUDE <SRCDIR>/wclass/builder.ctl ]
# IDE and browser require WGML/docs
[ INCLUDE <SRCDIR>/wgml/builder.ctl ]
[ INCLUDE <SRCDIR>/ide/builder.ctl ]
[ INCLUDE <SRCDIR>/ide/bviper/builder.ctl ]
[ INCLUDE <SRCDIR>/idebatch/builder.ctl ]
[ INCLUDE <SRCDIR>/brgen/builder.ctl ]
[ INCLUDE <SRCDIR>/browser/builder.ctl ]
# Miscellaneous bits that need copying over
[ INCLUDE <SRCDIR>/misc/builder.ctl ]
[ INCLUDE <SRCDIR>/redist/builder.ctl ]
[ INCLUDE <SRCDIR>/examples/builder.ctl ]
# Copy and pre-build the IDE demo
[ INCLUDE <SRCDIR>/idedemo/builder.ctl ]
# Libraries needed by installer
[ INCLUDE <SRCDIR>/zlib/builder.ctl ]
[ INCLUDE <SRCDIR>/ziplib/builder.ctl ]
