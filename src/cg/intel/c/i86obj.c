...
/*****************************************/
{
    ...
    /*
     * Always reset the used count. Sometimes we get into a pathological
     * case where an included file does not produce any line numbers due
     * to optimizations (inlining) and obj->line_info is false.
     */
    if( obj->lines )
        obj->lines->used = 0;
}
