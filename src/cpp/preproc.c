...
static void PP_TimeInit( void )
{
    ...
    sprintf( PP__TIME__, "\"%.2d:%.2d:%.2d\"", tod->tm_hour, tod->tm_min,
                                    tod->tm_sec );
    /* Casting day/year to unsigned to convince gcc printf format
     * checker that no, tm_mday and tm_year can not in fact be negative.
     */
    sprintf( PP__DATE__, "\"%3s %2d %d\"", Months[ tod->tm_mon ],
             (unsigned)tod->tm_mday, (unsigned)tod->tm_year + 1900 );
}
