 ...
 * NB: In OS/2 and NT cmd.exe, parentheses '(' and ')' may or may not
 * be metacharacters depending on context. Consider the following:
 *
 *  (echo foo)   <- metacharacters
 *  echo (foo)   <- regular characters
 *
 * On UNIX, parentheses are always metacharacters. A common instance
 * of non-metacharacter parentheses is 'C:\Program Files (x86)' on
...
#if !defined( __UNIX__ )
    /* Is the first non-whitespace character '(' ? */
    if( *SkipWS( (char *)cmd ) == SHELL_LPAREN ) {
        return( TRUE );
    }
#endif
    return( FALSE );
