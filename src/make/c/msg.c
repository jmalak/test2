...
/**************************************************/
{
    ...
    if( logFH != -1 ) {
        cbw = write( logFH, buff, len );
        /* If a write to the console failed, we can't very well
         * tell the user about it. So, suck it up and avoid compiler warning.
         */
        if( cbw != len )
            return;
    }
