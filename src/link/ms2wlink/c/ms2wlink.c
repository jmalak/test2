...
// generate a watcom linker command file & write it to the screen.
{
    ...
    // A long time ago wlink was case insensitive by default like MS LINK,
    // but now we have to explicitly turn case sensitivity off, unless
    // the /NOIGNORECASE switch was used.
    if( CaseSensitive ) {
        CommandOut( "option caseexact" );
    } else {
