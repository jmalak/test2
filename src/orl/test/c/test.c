...
/****************************************************/
{
    ...
    printf( " %4.4x ", (unsigned)ORLSymbolGetValue( symbol ) );
...
/*******************************************/
{
    printf( " tag=%8.8x ", (unsigned)reloc->offset );
    if( reloc->type < sizeof( relocTypes ) / sizeof( *relocTypes ) ) {
        printf( "%-8s", relocTypes[reloc->type] );
    } else {
        printf( "??? (%2.2x)", (unsigned)reloc->type );
    }
    printf( " addend=%8.8x ", (unsigned)reloc->addend );
    if( reloc->symbol ) {
...
/**********************************************/
{
    ...
    if( dump.sections ) {
        //printf( "\nSection Handle:\t0x%x\n", o_shnd );
        printf( "Size=%8.8x ", (unsigned)size );
        sec_type = ORLSecGetType( o_shnd );
        printf( "Align=%4.4x ", (unsigned)ORLSecGetAlignment( o_shnd ) );
