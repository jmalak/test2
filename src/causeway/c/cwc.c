...
/****************************************/
{
    ...
    // create temp file in current directory; tmpnam() might create a temp file
    // on a different file system, causing rename() to fail.
    fi = mkstemp( temp_fname );
    if( fi < 0 ) {
