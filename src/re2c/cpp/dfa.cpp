...
*  ========================================================================
*
* Description:  DFA class for re2c.
*
****************************************************************************/
...
DFA::DFA(Ins *ins, uint ni, uint lb, uint ub, Char *rep)
    ...
    for(iP = s->kernel; (i = *iP) != NULL; ++iP){
        if(i->i.tag == CHAR){
        for(Ins *ji = i + 1; ji < (Ins*) i->i.link; ++ji){
            if((ji->c.link = goTo[ji->c.value - lb].to) == 0)
            goTo[nGoTos++].ch = (Char)ji->c.value;
            goTo[ji->c.value - lb].to = ji;
        }
