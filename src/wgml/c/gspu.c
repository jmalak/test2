...
static  errno_t open_pu_file( int n )
{
    ...
    if( n > 0 && n < 10 ) {
        if( workfile[n - 1] == NULL ) {   // not yet open
            filename[7] = '0' + n;
#ifdef _MSC_VER
            /* Microsoft does not support the 'u' modifier which is
             * defined by ISO/IEC TR 24731-1:2007 to produce the same
             * access permissions as fopen(). Without the 'u', only
             * the current user can access the file.
             */
            erc = fopen_s( &workfile[n - 1], filename, "wt" );
#else
