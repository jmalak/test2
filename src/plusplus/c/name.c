...
/* Carefully read the last 1-3 bytes of a string
 * without potentially accessing invalid memory.
 */
static unsigned getLastBytes( void const *str )
/*********************************************/
{
    ...
    for( i = 0, rem = 0; i < sizeof( unsigned ); ++i ) {
        if( s[i] )
            rem |= (unsigned)s[i] << (i * 8);
        else
            break;
    }
    return( rem );
...
#else
    return( getLastBytes( s1 ) - getLastBytes( s2 ) );
#endif
}
