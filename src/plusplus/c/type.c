...
//  curr_unmod =
    TypeModFlagsBaseEC( curr_type, &curr_flags, &curr_base );
...
static PTREE nameOfId( PTREE id )
{
    ...
    // Uncommenting the following debug check fixes a problem
    // with gcc 11.4.8 when optimizing verifyQualifiedId().
    // It is not at all clear why uncommenting the check makes
    // a difference.
//#ifndef NDEBUG
...
/***************************************************************/
{
    ...
    if( dinfo->id != NULL ) {
        ...
        msg_num = ERR_CALL_WATCOM;  // Shut up warnings
...
// If the following function is compiled with -O2 and above
// using gcc 11.4.8, the resulting C++ compiler produces spurious
// errors. However, see comment in nameOfId() above.
...
static void saveType( void *e, carve_walk_base *d )
{
    ...
    // Following assignments only shut up warnings
    save_sym = NULL;
    save_scope = NULL;
    save_info = NULL;
    save_type = NULL;
