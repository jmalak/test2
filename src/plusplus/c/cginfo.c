    ...
    void *parm )                // - parameter
{
    ...
    case MSG_BAD_PARM_REGISTER:
        CErr2( ERR_BAD_PARM_REGISTER, (intptr_t)parm );
        break;
    ...
    case MSG_BACK_END_ERROR:
        CErr2( ERR_BACK_END_ERROR, (uintptr_t)parm );
        break;
    case MSG_BAD_SAVE:
...
// Return symbol name related info for object file.
    ...
    case EXTN_PRMSIZE:
        return( (char *)(uintptr_t)GetParmsSize( sym ) );
    default:
        return( NULL );
    }
}
        ...
        aux_class request )     // - request
{
    ...
    case PROEPI_DATA_SIZE:
        DbgNotSym();
        DbgNotRetn();
        retn = (void *)(uintptr_t)ProEpiDataSize;
        break;
#if _CPU == 386
    case P5_PROF_DATA:
        DbgNotSym();
    ...
    case CLASS_NAME:
        DbgNotSym();
        DbgNotRetn();
        if(((fe_seg_id)(intptr_t)sym) == SEG_CODE ) {
            retn = CodeClassName;
        } else {
            retn = SegmentClassName( (fe_seg_id)(intptr_t)sym );
        }
        break;
#endif
#if _INTEL_CPU
    ...
    case TEMP_LOC_TELL :
        DbgNotSym();
        DbgNotRetn();
        CgBackDtorAutoOffset( dtor_sym, (uintptr_t)sym );
        break;
    ...
    case PEGGED_REGISTER :
        DbgNotSym();
        DbgNotRetn();
        retn = SegmentBoundReg( (fe_seg_id)(intptr_t)sym );
        break;
#endif
