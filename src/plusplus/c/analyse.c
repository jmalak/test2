...
*  ========================================================================
*
* Description:  Analyze parsed tree of tokens, issue semantic errors,
*               convert parse tree into a form that can be directly
*               emitted using IcEmitExpr.
*
****************************************************************************/
        ...
        if( ret != CMP_VOID ) {
            ...
            if( PTreeWarnExpr( expr,
                ...
                if( NumSign(op1_size) ) { // signed
                    sprintf( num1, "%lld", low.u._64[0] );
                    sprintf( num2, "%lld", high.u._64[0] );
                    CErr( INF_SIGNED_TYPE_RANGE, op1->type, num1, num2 );
                } else {
                    sprintf( num1, "%llu", low.u._64[0] );
                    sprintf( num2, "%llu", high.u._64[0] );
                    CErr( INF_UNSIGNED_TYPE_RANGE, op1->type, num1, num2 );
                }
                if( NumSign(result_size) ) {
                    sprintf( num1, "%lld", val.u._64[0] );
                    CErr( INF_SIGNED_CONST_EXPR_VALUE, num1 );
                } else {
                    sprintf( num1, "%llu", val.u._64[0] );
...
//      addr_left = &orig->u.subtree[0];
    ...
    for( ap = opcomb_actions[ action_code ]; *ap < OPAC_END ; ++ap ) {
        ...
        // analysis.
        // NB: The following self-assignment shuts up GCC uninitialized variable
        // warning; opac_memb_ptr_ext may *not* be changed here, the code below
        // relies on its value being retained after a 'continue'.
        OPAC* opac_memb_ptr_ext = opac_memb_ptr_ext;    // - operator-specific analysis string
...
start_opac_string:
        ...
        switch( *ap ) {
          ...
          { TYPE refed = TypeReferenced( type );
            PTREE node;
            CGOP new_op = PTO_UNARY_CPP;   // initialized to silence warning
                    ...
                    if( ref_right == ref_left ) {
                        ...
                        switch( TypeCommonDerivation( ref_left, ref_right ) ) {
                          ...
                          case CTD_RIGHT_VIRTUAL :
                           ...
                           force_rvalue = FALSE;
                           break;
                          DbgDefault( "ANALYSE -- invalid type common derivation" );
                             force_rvalue = TRUE; // silence warning
                        }
                    }
                } else {
                    force_rvalue = TRUE;
