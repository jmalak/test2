    ...
    SYMBOL sym )                // - sym
{
    ...
    if( sym == file_ctl->opt_retn && CgRetnOptActive( fctl ) ) {
        if( file_ctl->s.opt_retn_val ) {
            ...
            cgname = NULL; // TODO: Is this case valid?
        }
    } else {
        sym = SymTrans( sym );
        sym = finalAlias( sym );
    ...
    call_handle handle )        // - handle for call when gen'ing inline fun.
{
    ...
    cg_name op2 = NULL;         // - operand [2]
    cg_name op3;                // - operand [3]
    cg_type indexing_type;      // - type of array being indexed
    unsigned ptr_offset = 0;    // - offset set by IC_DATA_PTR_OFFSET
    unsigned data_size  = 0;    // - size set by IC_DATA_SIZE
    segment_id curr_seg = 0;    // - current segment
    cg_op cg_opcode;            // - opcode for code generator
    SRCFILE current_src = NULL; // - current source file
    ...
    target_offset_t vbr_exact  = 0; // exact for virtual base
    target_offset_t vbr_delta  = 0; // delta for virtual base
    target_offset_t vbr_offset = 0; // offset to virtual base ptr.
    target_offset_t vbr_index  = 0; // index for virtual base
    ...
    target_offset_t vf_offset = 0;  // offset to virtual function ptr
    target_offset_t vf_index  = 0;  // index for virtual function
    cg_name vf_ptr = NULL;      // pre-computation of vf_ptr
    SYMBOL vf_this = NULL;      // this for function (bound ?)
                                // IC_CATCH_xxx
    SE* try_se = NULL;          // try SE for catch
    SE* catch_se = NULL;        // catch SE
                                // IC_DLT_DTOR_xxx
    target_size_t elem_size = 0;// size of element
    SE* se_dlt = NULL;          // SE entry created
                                // IC_INIT_REF_xxx
    cg_name set_expr = NULL;    // expression for set
    ...
    dtor_kind = 0;
    dtor_last_reqd = NULL;
    exprn_type = lvalue_type = indexing_type = TY_LAST_FREE; // invalid value
    for( ; ; ) {
...

        {
          ...
          { label_handle *lab;              // - next label handle
            ...
            dump_label( printf( "LabCs Def %d %p\n"
                              , (int)(fctl->base_labs_cs + ins_value.uvalue)
                              , *lab ) );
          } break;
          ...
          { label_handle *label;            // - goto label
            ...
            dump_label( printf( "LabGoto Def %d %p\n"
                              , (int)(fctl->base_goto_near + ins_value.uvalue)
                              , *label ) );
          } break;
...
//          Virtual Base reference with inlined args
//
          // uses vbr_xxx variables
          case IC_VB_EXACT :        // SET vbr_exact
            vbr_exact = ins_value.uvalue;
            break;

          case IC_VB_DELTA :        // SET vbr_delta
            vbr_delta = ins_value.uvalue;
            break;

          case IC_VB_OFFSET :       // SET vbr_offset
            vbr_offset = ins_value.uvalue;
            break;

          case IC_VB_INDEX :        // SET vbr_index
            vbr_index = ins_value.uvalue;
            break;
