...
/********************************************/
{
    ...
    case 'a':
        ...
        if( tolower( *arg ) == 'd' ) {
            ...
            arg++;
            if( *arg == '=' ) arg++;
            strlcpy( CmdLineParms.PrependString, arg, _MAX_PATH );
            CmdLineParms.PrependString[_MAX_PATH - 1] = '\0';
    ...
    case 'f':
        ...
        case 'o':
            arg++;
            if( *arg == '=' ) arg++;
            strlcpy( CmdLineParms.OutResFileName, arg, _MAX_PATH );
...
/****************************/
{
    ...
    if( CmdLineParms.InExeFileName[0] == '\0' ) {
        if (CmdLineParms.NoResFile) {
            strlcpy( CmdLineParms.InExeFileName, CmdLineParms.InFileName,
                        _MAX_PATH );
        } else {
    ...
    if( CmdLineParms.OutExeFileName[0] == '\0' ) {
        strlcpy( CmdLineParms.OutExeFileName,
