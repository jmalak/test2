...
/*****************************/
{
    ...
    while( ( alias = GetAliasData( first ) ) != NULL ) {
        ...
        *new = len1;
        new++;
        memcpy( new, alias, len1 );
        new += len1;
        *new = len2;
        new++;
        memcpy( new, subst, len2 );
        new -= len1 + 2;
...
/*************************/
{
    /* The actual codebuf needs to be larger than the threshold because
     * there could be fixups that need to be added. See asmglob.h.
     */
    uint_8              codebuf[ MAX_LEDATA_THRESHOLD + 12 ];
