...
/* Originally the code used NULL as the default string. However,
 * passing NULL to a %s printf() format specifier invokes undefined
 * behavior. Some run-times print nothing, some print (null), others
 * might do something else.
 */
static char     EmptyStr[] = "";

/* Defaults for all output values */
static char     *DefType   = EmptyStr;
static char     *DefRedist = EmptyStr;
static char     *DefDir    = EmptyStr;
static char     *DefUsr    = EmptyStr;
static char     *DefRel    = EmptyStr;
static char     *DefCond   = EmptyStr;
static char     *DefPack   = EmptyStr;
static char     *DefWhere  = EmptyStr;
static char     *DefDesc   = EmptyStr;
static char     *DefOld    = EmptyStr;
static char     *DefPatch  = EmptyStr;
static char     *DefDstvar = EmptyStr;
...
static bool ContainsWord( const char *str, const char *word )
{
    ...
    /* FirstWord writes an extra '\0' beyond the end of the
     * string; we have to allocate one more byte than what
     * strdup() would do.
     */
    s_copy = malloc( strlen( str ) + 2 );
    strcpy( s_copy, str );
    p = FirstWord( s_copy );
    do {
        if( !strcmp( p, word ) ) {
...
static void ProcessLine( const char *line )
{
    ...
    if( !special ) {
        ...
        if( !Product || (where == EmptyStr) || ContainsWord( where, Product ) ) {
...
static void ProcessDefault( const char *line )
{
    ...
    if( DefType   != EmptyStr ) free( DefType );
    if( DefRedist != EmptyStr ) free( DefRedist );
    if( DefDir    != EmptyStr ) free( DefDir );
    if( DefUsr    != EmptyStr ) free( DefUsr );
    if( DefRel    != EmptyStr ) free( DefRel );
    if( DefCond   != EmptyStr ) free( DefCond );
    if( DefPack   != EmptyStr ) free( DefPack );
    if( DefWhere  != EmptyStr ) free( DefWhere );
    if( DefDesc   != EmptyStr ) free( DefDesc );
    if( DefOld    != EmptyStr ) free( DefOld );
    if( DefPatch  != EmptyStr ) free( DefPatch );
    if( DefDstvar != EmptyStr ) free( DefDstvar );
    DefType = DefRedist = DefDir = DefUsr = DefRel = DefCond
    = DefPack = DefWhere = DefDesc = DefOld = DefPatch = DefDstvar = EmptyStr;
