...
*  ========================================================================
*
* Description:  The CheckedBufferedFile class for the dlgprs tool.
*
****************************************************************************/
...
#ifdef _MSC_VER
    // Microsoft's runtime may not fill the buffer when reading text files;
    // it will read BUFFERSIZE bytes from the file, but only return
    // BUFFERSIZE minus any CR characters removed.
    // Try to fill up the buffer entirely, like the Watcom runtime does.
    while( _bufLen < BUFFERSIZE ) {
        int to_read = BUFFERSIZE - _bufLen;
        int num_read = CheckedFile::read( &_buffer[_bufLen], to_read );
        if( num_read > 0 )
            _bufLen += num_read;
        else
            break;
